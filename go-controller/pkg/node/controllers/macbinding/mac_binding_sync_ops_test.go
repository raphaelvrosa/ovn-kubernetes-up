...
func newSBTestHarnessWithMACBinding(t *testing.T, setup libovsdbtest.TestSetup) libovsdbclient.Client {
	t.Helper()
	...
	_, err = sbClient.Monitor(context.Background(),
		sbClient.NewMonitor(libovsdbclient.WithTable(&sbdb.MACBinding{})))
	if err != nil {
		t.Fatalf("failed to monitor MAC_Binding: %v", err)
	}
	return sbClient
}

func testSBMACBinder(t *testing.T, newBinder func(libovsdbclient.Client) macbinding.MacBindingSyncOps, ip string) {
	t.Helper()
	...
	t.Run("AddMACBinding single port", func(t *testing.T) {
		sbClient := newSBTestHarnessWithMACBinding(t, libovsdbtest.TestSetup{
			SBData: datapaths,
		})

		binder := newBinder(sbClient)
		ports := []macbinding.PortInfo{{LogicalPort: "lsp1", DatapathUUID: dpUUID1}}
		...
		if !match || err != nil {
			t.Fatal(fmt.Errorf("data mismatch: %s", matcher.FailureMessage(sbClient)))
		}
	})
	...
	t.Run("Update replaces MAC binding", func(t *testing.T) {
		...
		sbClient := newSBTestHarnessWithMACBinding(t, initialData)
...
func TestIPv4SBMACBinder(t *testing.T) {
	testSBMACBinder(t, func(client libovsdbclient.Client) macbinding.MacBindingSyncOps {
		return macbinding.NewMACBindingSyncOps(client, nil)
	}, "192.168.1.10")
}
...
func TestEnsureARPFlow(t *testing.T) {
	...
	syncer := macbinding.NewMACBindingSyncOps(nil, mock)
