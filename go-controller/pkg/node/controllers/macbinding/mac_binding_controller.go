...
// resolve without depending on recieving ARP/ND replies, which is what we can't
...
//   - From SBDB: external GR port binging additions or deletions
...
type MACBindingController struct {
	...
	// macBindingSourceForUplinks memoizes the uplink->source designation. A nil
	// value means it is invalidated and a full reconcile is due to recompute
	// source->follower allocation (at startup or after ReconcileUplinkSource);
	// see hasUnknownSource.
...
// reconcileMacBindingsUpdate handles the add/MAC-change path
// (macBindingReconciler). A binding that appeared or changed must reach
// followers promptly, so it warns about a backlog after a short delay.
...
// reconcileMacBindingsRefresh handles the periodic timestamp-refresh path
// (macBindingRefreshReconciler), which is less urgent than an add or change and
// so tolerates more staleness before warning about a backlog.
...
// reconcileMacBindingsForFollower mirrors all of a follower's source bindings
// onto it. Used when a follower is newly allocated to a source and needs to
// catch up on the bindings already resolved there.
...
// reconcileNetwork reacts to a Network/NAD change.
