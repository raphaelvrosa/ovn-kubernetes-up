...
func initClusterEgressPoliciesOps(nbClient libovsdbclient.Client, addressSetFactory addressset.AddressSetFactory, ni util.NetInfo,
	clusterSubnets []*net.IPNet, controllerName, routerName string, ops []ovsdb.Operation) ([]ovsdb.Operation, error) {
	...
	// Ensure address sets exist (these are idempotent, not batched)
...
func ensureClusterNodeAddressSets(addressSetFactory addressset.AddressSetFactory,
	nodeLister listers.NodeLister, v4, v6 bool) (addressset.AddressSet, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get node addresses: %v", err)
	}
	...
	if err = as.SetAddresses(util.StringSlice(allAddresses)); err != nil {
		return nil, fmt.Errorf("unable to set IPs to no re-route address set %s: %w", NodeIPAddrSetName, err)
	}

	return as, nil
}

// ensureDefaultNoRerouteNodePoliciesOps is the ops-building version of ensureDefaultNoRerouteNodePolicies
// It batches node policy creation
