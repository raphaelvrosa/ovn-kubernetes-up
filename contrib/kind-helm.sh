...
usage() {
    ...
    echo "-udnp| --udn-proxy-enable                     UDN Proxy enabled. DEFAULT: Disabled"
    ...
    while [ "$1" != "" ]; do
        case $1 in
            ...
            -nse | --network-segmentation-enable) ENABLE_NETWORK_SEGMENTATION=true
                                                  ;;
            -udnp | --udn-proxy-enable)           ENABLE_UDN_PROXY=true
                                                  ;;
...
print_params() {
     ...
     echo "ENABLE_UDN_PROXY = $ENABLE_UDN_PROXY"
...
helm upgrade --install ovn-kubernetes . -f "${value_file}" ${extra_values_args} \
          ...
          --set global.enableUDNProxy=$(if [ "${ENABLE_UDN_PROXY}" == "true" ]; then echo "true"; else echo "false"; fi) \
