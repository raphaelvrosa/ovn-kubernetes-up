...
var _ = Describe("Network Segmentation: ARP Proxy Flow Counter Regression", feature.NetworkSegmentation, func() {
	...
	It("should not produce duplicate ARP replies from multiple GRs for inbound broadcast", func() {
		...
		startTcpdumpMonitorPodOnNode(f, proxyCmdRetryTimeout, flowCounterMonitorPod,
		...
		Expect(arpReplies).To(BeNumerically("==", 3),
			"expected exactly 3 ARP reply for node IP %s, got %d — each GR is generating a duplicate reply", nodeIPv4, arpReplies)

		By("asserting: only 0 patch egress counter should not have incremented")
		Expect(patchesWithDelta).To(BeNumerically("==", 0),
			"expected only 0 patch port to emit an ARP reply, but %d patch ports had egress counter increases", patchesWithDelta)
	})
})
...
var _ = Describe("Network Segmentation: NDP Proxy Flow Counter Regression", feature.NetworkSegmentation, func() {
	...
	It("should not produce duplicate NDP NA from multiple GRs for inbound multicast NS", func() {
		...
		startTcpdumpMonitorPodOnNode(f, proxyCmdRetryTimeout, ndpFlowCounterMonitorPod,
			targetNodeName, briface, "-n -vv -l", quotedFilter)
...
var _ = Describe("Network Segmentation: ARP Proxy Controller", feature.NetworkSegmentation, func() {
	...
	DescribeTable("should manage MAC_Binding lifecycle for IPv4",
		func(udnType udnProxyTestType) {
			...
			Expect(targetNodeIPv4).NotTo(BeEmpty(), "target node %s must have an IPv4 InternalIP", targetNodeName)
			nodeIPv4 = targetNodeIPv4[0]
			framework.Logf("target node: name=%s, IPv4=%s", targetNodeName, nodeIPv4)
			...
			By("verifying ALL UDN GRs have MAC_Binding entries propagated for the external container IPv4")
			for _, udn := range extraUDNs {
				udnGRExtPort := proxyUDNGatewayRouterExtPort(udn.namespace, udn.name, targetNodeName)
				Eventually(checkProxyMACBindingExists(f, ovnkPod, udnGRExtPort, extIPv4, extMAC),
					macBindingStabilizationTime, proxyPollInterval).Should(Succeed())
			}
			...
			By("pinging the node from the external container via IPv4 (triggers new ARP resolve)")
			pingOutput, err := infraprovider.Get().ExecExternalContainerCommand(extContainer, []string{
				"ping", "-c", "3", "-W", "2", nodeIPv4,
			})
			framework.ExpectNoError(err, "ping from external container to node %s failed: %s", nodeIPv4, pingOutput)
			...
			Eventually(checkProxyMACBindingExists(f, ovnkPod, defaultGRExtPort, extIPv4, extMAC),
				macBindingStabilizationTime, proxyPollInterval).Should(MatchError(ContainSubstring("does not contain MAC")))
			...
			for _, udn := range extraUDNs {
				udnGRExtPort := proxyUDNGatewayRouterExtPort(udn.namespace, udn.name, targetNodeName)
				Eventually(checkProxyMACBindingExists(f, ovnkPod, udnGRExtPort, extIPv4, proxyMigratedMAC),
					macBindingStabilizationTime, proxyPollInterval).Should(Succeed())
			}
...
var _ = Describe("Network Segmentation: NDP Proxy Controller", feature.NetworkSegmentation, func() {
	...
	DescribeTable("should propagate MAC_Binding entries to all UDN GRs for IPv6",
		func(udnType udnProxyTestType) {
			...
			Expect(targetNodeIPv6).NotTo(BeEmpty(), "target node %s must have an IPv6 InternalIP", targetNodeName)
			nodeIPv6 = targetNodeIPv6[0]
			framework.Logf("target node: name=%s, IPv4=%s", targetNodeName, nodeIPv6)
			...
			By("verifying the kernel IPv6 neighbor table has an entry for the external container")
			Eventually(checkProxyKernelNDPEntry(targetNodeName, extIPv6, extMAC),
				macBindingStabilizationTime, proxyPollInterval).Should(Succeed())
			...
			By("verifying kernel IPv6 neighbor entry is re-created after resumed traffic")
			Eventually(checkProxyKernelNDPEntry(targetNodeName, extIPv6, extMAC),
				macBindingStabilizationTime, proxyPollInterval).Should(Succeed())
			...
			By("pinging the node from the external container via IPv6 (triggers new ARP resolve)")
			pingOutput, err := infraprovider.Get().ExecExternalContainerCommand(extContainer, []string{
				"ping", "-6", "-c", "3", "-W", "2", nodeIPv6,
			})
			framework.ExpectNoError(err, "ping from external container to node %s failed: %s", nodeIPv6, pingOutput)
			...
			By("verifying kernel IPv6 neighbor entry is updated with the new MAC")
			Eventually(checkProxyKernelNDPEntry(targetNodeName, extIPv6, proxyMigratedMAC),
				macBindingStabilizationTime, proxyPollInterval).Should(Succeed())
...
func checkProxyKernelNDPEntry(nodeName, ip, mac string) func() error {
	return func() error {
		output, err := infraprovider.Get().ExecK8NodeCommand(nodeName, []string{"ip", "-6", "neigh", "show", ip})
		if err != nil {
			return fmt.Errorf("failed to query IPv6 neighbor table on node %s for ip=%s: %w", nodeName, ip, err)
		}
		if !strings.Contains(strings.ToLower(output), strings.ToLower(mac)) {
			return fmt.Errorf("kernel IPv6 neighbor entry for ip=%s on node %s does not contain MAC %s, got: %s", ip, nodeName, mac, output)
		}
		return nil
	}
}
